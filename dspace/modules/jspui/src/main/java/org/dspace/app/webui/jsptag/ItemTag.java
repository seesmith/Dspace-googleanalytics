        ...
        try
        {
        	...
        	else
        	{
            	...
            	if (html)
            	{
                    ...
                    String url = request.getContextPath() + "/html/" + handle + "/"
                    ...
                    out.print(url + "\"");
                    out.print(" onClick=\"javascript: pageTracker._trackPageview('" + url + "'); ");
                    out.print("\">");
            		...
            		for (int i = 0; i < bundles.length; i++)
            		{
                        ...
                        for (Bitstream bitstream : bitstreams)
            			{
            				...
            				if (!bitstream.getFormat().isInternal())
            				{
                                ...
                                if ((handle != null) && (bitstream.getSequenceID() > 0))
                                {
                                    url = url + "/bitstream/"
                                            + item.getHandle() + "/"
                                ...
                                {
                                    url = url + "/retrieve/" + bitstream.getID() + "/";
                                }

                                url = url + UIUtil.encodeBitstreamName(bitstream.getName(),
                                        Constants.DEFAULT_ENCODING);
                                bsLink += url + "\"";
                                bsLink += " onClick=\"javascript: pageTracker._trackPageview('" + url + "'); ";
                                bsLink += "\">";
